        ...
        <div class="card position-relative">
            <ul class="list-group list-group-flush">
                <?php
                   ...
                   $sql = "SELECT * FROM user WHERE `name` LIKE '$search_name%'";
                   $result = mysqli_query($conn,$sql);
                   if(mysqli_num_rows($result)){
                   while($row = mysqli_fetch_assoc($result)){
                    $sno = $row['sno'];
                    $name = $row['name'];
                    echo "<li class='list-group-item'><span style='margin-right:8px;'><i class='fa-solid fa-user'></i></span><a class='link-dark' href='profile.php?sno=$sno' style='font-size:19px;text-decoration:none;'>$name</a><span style='float:right;'><form  action='_friendrequest.php' method='post'><button type='submit' class='btn btn-primary' name='friend' value = '$sno'>Add Friend</button></form></span></li>";
                   }
                   }else{ echo "<h3 style='margin:auto;'>No Match Found</h3>";
                }
